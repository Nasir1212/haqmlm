...
Route::get('/matrix', function () {
    ...
    // Main Logic for Matrix Auto-Generation
...
// Sort users based on PV check
$remainingElements = [];
$movingElements = [];

foreach ($userIds as $user) {
    if ($user == 1) continue; // Skip root user
    
    // Check if user has submitted PV in the previous month
    $current_user = User::where('id',$user)->first();
    
    if ($current_user->submit_check == 0) {
        $movingElements[] = $user; // Move users who didn't give PV to the bottom
    } else {
        $remainingElements[] = $user; // Keep users with PV at the top
    }
}

// Merge the sorted users (users with PV first, then others)
...
// Update Uplevel Function
function UpdateUplevel($fix, $id)
{
    $limit = 10; // Stop at level 10
    $start = 1;
    ...
    while ($id != "" || $id != "0") {
        if (isUserExists($id)) {
            ...
            $lv = 'lv_'.$start;
            $data  = json_decode($parent->$lv, true);
            $data[] = $fix;
            ...
            // 🚨 If the user crosses level 10, move them further down
            if ($start > $limit) {
                moveUserToBottom($fix);
                break;
            }
        } else {
            break;
...
// Move User to Bottom if they exceed level 10
function moveUserToBottom($userId) {
    // Find a parent at the lowest available position
    $parent = UserExtra::where('fill_check', 3)->latest('id')->first();
    if (!$parent) {
        $parent = UserExtra::where('fill_check', 2)->latest('id')->first();
    }
    ...
    if ($parent) {
        $newUser = UserExtra::where('user_id', $userId)->first();
        $newUser->pos_id = $parent->user_id;
        $newUser->root_level = $parent->root_level + 1;
        $newUser->save();

        // Increase parent's fill check
        $parent->fill_check += 1;
        $parent->save();
    }
}
...
// Parent Capacity Level Function
