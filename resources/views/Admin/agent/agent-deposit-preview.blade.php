                    ...
                    <div class="card-body card-body-deposit">
                        <ul class="list-group text-center">
                            ...
                            <p class="list-group-item">
                                Amount:
                                <strong>{{ $_GET['request_amount']}} </strong> USD
                            </p>
                            <p class="list-group-item">
                                Charge:
                            ...
                            <p class="list-group-item">
                                Payable: <strong> {{ $_GET['payable_amount']}}</strong> {{ $_GET['supported_currencies']}}
                            </p>
                            <p class="list-group-item">
                                Conversion Rate: <strong>1 USD = {{ $_GET['rate']}}  {{ $_GET['supported_currencies']}}</strong>
                            </p>
                           
                            </ul>
							<a href="{{ route('Deposit_form') }}" class="btn btn-secondary font-weight-bold" >Close</a>
                            <button type="button" data-pay_account="{{ $_GET['pay_ac_id'] }}" class="btn btn-success font-weight-bold deposit-confirm" data-toggle="modal" data-target="#depositModal">Confirm Deposit</button>
...
<div class="modal fade" id="depositModal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-modal="true" style="padding-right: 17px;">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			...
			<form action="{{ route('agentFormSubmit')}}" method="post" enctype="multipart/form-data">
				@csrf
						...
						<div class="card-body">
								<div class="row">
									<div class="col-md-12 text-center">
										<p class="text-center mt-2">You have requested  <b class="text--success">{{ $_GET['request_amount']}} USD</b>, Please pay  <b class="text--success"> {{ $_GET['payable_amount']}} {{ $_GET['supported_currencies']}}</b> for successful payment                                </p>
