...
@section('content')
	<div class="main-container">
		...
		<div class="page-header">
			<!-- Breadcrumb start -->
			<ol class="breadcrumb">
				<li class="breadcrumb-item">Update Package Category</li>
			</ol>
			<!-- Breadcrumb end -->
		</div>
		<!-- Page header end -->
					...
					<div class="card-body">
						<form action="{{ route('package_update')}}" method="post" enctype="multipart/form-data">
							...
							<div class="form-group">
								...
								<input type="text" class="form-control " placeholder="Package Name" id="package_name" name="package_name" value="{{ $package->name }}">
							</div>
							<div class="form-group">
								<label for="package_slug" class="w-100 font-weight-bold mb-1">Package Slug <span class="text-warning">(optional)</span></label>
								<input type="text" class="form-control " placeholder="Package slug" id="package_slug" name="package_slug" value="{{ $package->slug }}">
							</div>
							<div class="form-group">
								...
								<input type="text" class="form-control " placeholder="Package Regular price" id="package_regular_price" name="package_regular_price"  value="{{ $package->regular_price }}">
							</div>

							<div class="form-group">
								<label for="package_sell_price" class="w-100 font-weight-bold mb-1">Package Sell Price <span class="text-danger">*</span></label>
								<input type="text" class="form-control " placeholder="Package sell price" id="package_sell_price" name="package_sell_price" value="{{ $package->main_price }}">
							</div>

							<div class="form-group">
								<label for="package_point" class="w-100 font-weight-bold mb-1">Package Point <span class="text-danger">*</span></label>
								<input type="text" class="form-control " placeholder="Package point" id="package_point" name="package_point"  value="{{ $package->point }}">
							</div>
							<div class="form-group">
								<label for="package_spcd" class="w-100 font-weight-bold mb-1">Package Shipping In Dhaka <span class="text-danger">*</span></label>
								<input type="text" class="form-control " placeholder="Package shipping cost amount" id="package_spcd" name="package_spcd" value="{{ $package->shipping_cost_in_dhaka }}">
							</div>
							
							<div class="form-group">
								<label for="package_spco" class="w-100 font-weight-bold mb-1">Package Shipping Out Dhaka <span class="text-danger">*</span></label>
								<input type="text" class="form-control " placeholder="Package shipping cost amount" id="package_spco" name="package_spco" value="{{ $package->shipping_cost_out_dhaka }}">
							</div>
							<div class="form-group">
								<label for="package_details" class="w-100 font-weight-bold mb-1">Package details <span class="text-danger">*</span></label>
								<textarea  class="form-control "  id="package_details" name="package_details">{{ $package->details }}</textarea>
							</div>

							<div class="form-group">
								<label for="product List" class="w-100 font-weight-bold mb-1">Product List <span class="text-danger">*</span></label>
								@foreach ($products as  $product)
								<input type="checkbox" name="product_code[]" @if(in_array($product->id,$package_product_ids)) checked @endif value="{{ $product->id}}"> &nbsp; <div class="d-inline-block p-2 border">{{ $product->name }} <div class="badge">{{ $product->brand->name }}</div></div> 
								@endforeach
							...
							<div class="col-12">
								<div class="form-group">
									<button type="button" id="gallery_browse" class="btn btn-primary">Image Gallery</button><input type="text" class="form-control d-inline-block" name="img" id="img" value="{{ $package->img_name }}">
								</div>
							</div>
